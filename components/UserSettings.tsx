...
import { destroy, get, post, put } from "@/lib/server";
...
// Handles create/update with FormData: title, header, footer
type SavePayload = { formData: FormData; id?: string };
const isExistingLetterhead = (id?: string) =>
...
const UserSettings = () => {
  ...
  const handleCreateLetterhead = () => {
    ...
    if (letterheads.length === 0) {
      toast.success("Letterhead created successfully");
    } else {
      toast.success("Letterhead created successfully");
    }
  };
  ...
  const handleDeleteRequest = async (id: string) => {
    ...
    if (status) {
      ...
      queryClient.invalidateQueries({ queryKey: [queryNames.LETTER_HEADS] });
    } else {
      toast.error(
        message ||
          "Failed to delete template. Please try again or contact support.",
      );
    }
  };

  const deleteMutation = useMutation({ mutationFn: handleDeleteRequest });

  const handleDeleteLetterhead = (index: number) => {
    const letterhead = letterheads[index];
    if (!letterhead) return;

    // If it's from DB, delete permanently via API; otherwise remove from state only
    if (isExistingLetterhead(letterhead.id)) {
      setDeletingIndex(index);
      deleteMutation.mutate(letterhead.id, {
  ...
  const handleFormRequest = async ({ formData, id }: SavePayload) => {
    const isUpdate = isExistingLetterhead(id);
    const { status, message } = await (isUpdate ? put : post)({
      url: `${TEMPLATES_URL}/${isUpdate ? id : CREATE}`,
      payload: formData,
      isUploadDocuments: true,
    });
    if (status) {
      toast.success(
        message || (isUpdate ? "Template updated" : "Template saved"),
      );
      queryClient.invalidateQueries({ queryKey: [queryNames.LETTER_HEADS] });
    } else {
      toast.error(
        message ||
          (isUpdate ? "Failed to update template" : "Failed to save template"),
      );
    }
  };
  ...
  return (
    <>
                  ...
                  {letterheads.map((letterhead, index) => (
                    <LetterheadComponent
                      ...
                      onSave={(i) => {
                        ...
                        if (l.headerImage instanceof File)
                          fd.append("header", l.headerImage);
                        if (l.footerImage instanceof File)
                          fd.append("footer", l.footerImage);
                        setSavingIndex(i);
                        mutate(
                          { formData: fd, id: l.id },
                          {
                      ...
                      isPending={isPending && savingIndex === index}
                      deletePending={
                        deleteMutation.isPending && deletingIndex === index
                      }
                    />
                  ))}
                </div>
      ...
      <LetterheadPreviewModal
        ...
        letterhead={previewModal.letterhead as any}
      />
    </>
  );
};
